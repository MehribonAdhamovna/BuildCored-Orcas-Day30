...
try:
    ...
    from textual.containers import Container, Horizontal, Vertical, ScrollableContainer #type: ignore
...
class MorsePanel(Static):
    ...
    def compose(self) -> ComposeResult:
        yield Label("📟 Morse Decoder (Day 28)", id="morse-title")
        yield Label("Tap 'SPACE' in the box below to decode:", id="morse-hint")
        yield Input(placeholder="Tap spacebar here...", id="morse-tap-zone")
        yield Label("Decoded Message: ", id="morse-output")

    def on_input_changed(self, event: Input.Changed):
        # Every time a space is typed, we 'decode' it as a dot
        if event.value.endswith(" "):
            with state.lock:
                state.morse_text += "." 
                state.log("Morse: Signal received")
            # Clear the input for the next tap
            self.query_one("#morse-tap-zone", Input).value = ""
            self.refresh_morse()

    def refresh_morse(self):
        self.query_one("#morse-output", Label).update(f"Decoded Message: {state.morse_text}")
